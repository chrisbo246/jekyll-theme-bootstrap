    ...
    if (container && results.length) { // Are there any results?
      ...
      for (var i = 0; i < results.length; i++) {  // Iterate over the results
        ...
        */
        ...
        url='{{ post.url }}';
        filterTags='{%- include partials/post-tag-filter-classes.html post=post -%}'
      ...
      document.querySelector('#search-message').setAttribute('hidden','hidden');
    } else {
      container.innerHTML = html;
      document.querySelector('#search-message').removeAttribute('hidden');
    }
  }
  ...
  var init = function (options) {
    ...
    */
    ...
    idx = lunr(function () {
      ...
      } else if (options.langs && Array.isArray(options.langs)) {
        this.use(lunr.multiLanguage(options.langs));
      }
